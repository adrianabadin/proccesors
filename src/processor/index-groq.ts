/**
 * Procesador de ordenanzas con Groq API (Llama 3.1 8B Instant).
 * 
 * Usa la API OpenAI-compatible de Groq.
 * Base URL: https://api.groq.com/openai/v1
 * Rate limit: 30 RPM (2000ms entre llamadas)
 */
...
const RATE_LIMIT_MS = parseInt(process.env.RATE_LIMIT_MS ?? "3500", 10); // Seguro para 70B
...
/**
 * Llama a Groq API para analizar una ordenanza.
 */
...
async function persistExtraction(
  ...
  await db.transaction(async (tx) => {
    ...
    // 5. INSERT referencias_normativas (skip si no cumple constraint)
    ...
    if (extraction.montos_detallados.length > 0) {
      const montosValues = extraction.montos_detallados
        .filter((m) => m.valor !== null && m.concepto !== null) // Ensure non-null
        .map((m) => ({
          ordenanzaId,
          concepto: m.concepto!,
    ...
    if (extraction.anexos.length > 0) {
      const anexosValues = extraction.anexos
        .filter((a) => a.numero !== null) // Ensure non-null number
        .map((a) => ({
          ordenanzaId,
          numero: a.numero!,
          titulo: a.titulo,
...
async function main() {
  console.log("=== Procesador de Ordenanzas con Groq API ===\n");
  console.log(`Modelo: ${MODEL}`);
  console.log(`Batch size: ${PROCESS_ALL ? "TODAS" : BATCH_SIZE}`);
  console.log(`Rate limit: ${RATE_LIMIT_MS}ms entre llamadas (30 RPM)`);
  ...
  for (const ord of pendientes) {
    ...
    try {
      ...
      // Llamar a Groq API
      const extraction = await callGroqAPI(
        ord.numero,
        ord.anio,
        ord.textoCompleto,
      );
    ...
    // Rate limit entre llamadas (30 RPM = 2000ms)
