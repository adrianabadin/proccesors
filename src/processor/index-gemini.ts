/**
 * Procesador de ordenanzas con Gemini 2.5 Flash Lite.
 *
 * Usa la API de Google Generative AI con `responseMimeType: "application/json"`
 * para forzar salida estructurada (menor tasa de errores de parseo).
 *
 * Uso:
 *   BATCH_SIZE=5 npx tsx src/processor/index-gemini.ts          # test con 5
 *   npx tsx src/processor/index-gemini.ts                        # default 50
 *   npx tsx src/processor/index-gemini.ts --all                  # todas
 */
...
/**
 * Detecta si el error proviene de la API de Gemini (HTTP/cuota/auth)
 * vs un error de parseo o DB.
 */
function isGeminiAPIError(error: unknown): boolean {
  ...
  return (
    ...
    // El SDK de Gemini suele lanzar errores con status HTTP en el mensaje
    /\b[45]\d{2}\b/.test(error.message)
  );
}
...
/**
 * Llama a Gemini API para analizar una ordenanza.
 * Usa responseMimeType: "application/json" para forzar salida estructurada.
 */
...
async function main() {
  console.log("=== Procesador de Ordenanzas con Gemini 2.5 Flash Lite ===\n");
  ...
  for (const ord of pendientes) {
    ...
    try {
      ...
      const extraction = await callGeminiAPI(
        ord.numero,
        ord.anio,
        ord.textoCompleto,
      );
      ...
      if (isGeminiAPIError(error)) {
        ...
        console.log(`ERROR API: ${msg.slice(0, 200)}`);
        if (
          msg.toLowerCase().includes("429") ||
          msg.toLowerCase().includes("resource_exhausted") ||
          msg.toLowerCase().includes("quota")
        ) {
          console.log("  -> Rate limit/quota, esperando 60s...");
          await sleep(60_000);
        }
      } else {
