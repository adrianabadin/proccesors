/**
 * Procesador de ordenanzas con DeepSeek API (DeepSeek-V3).
 * 
 * Usa la API OpenAI-compatible de DeepSeek.
...
const RATE_LIMIT_MS = parseInt(process.env.RATE_LIMIT_MS ?? "1000", 10); // DeepSeek es rápido
...
/**
 * Llama a DeepSeek API para analizar una ordenanza.
 */
...
async function persistExtraction(
  ...
  await db.transaction(async (tx) => {
    ...
    // 6. INSERT montos (con validación de nulls)
    ...
    // 7. INSERT anexos (con validación de nulls)
...
async function main() {
  console.log("=== Procesador de Ordenanzas con DeepSeek-V3 ===\n");
  ...
  for (const ord of pendientes) {
    ...
    try {
      ...
      if (error instanceof APIError) {
        ...
        if (error.status === 429 || msg.includes("Insufficient Balance")) {
          console.log("  -> Quota/Balance issue, esperando 60s...");
          await sleep(60_000);
        }
      } else {
