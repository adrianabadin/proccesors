/**
 * Orquestador del procesamiento de ordenanzas con Gemini API.
 *
 * Flujo:
 * 1. Query ordenanzas sin procesar (procesado_ia = false)
 * 2. Para cada una, llamar a Gemini API con el prompt
 * 3. Parsear respuesta JSON con Zod
 * 4. En una transacción: UPDATE ordenanza + INSERT relaciones
 * 5. Rate limit entre llamadas
 *
 * Uso:
 *   BATCH_SIZE=5 npx tsx src/processor/index.ts          # test con 5
 *   npx tsx src/processor/index.ts                        # default 50
 *   npx tsx src/processor/index.ts --all                  # todas
 */
...
async function upsertEntidad(
  ...
  // Intentar encontrar entidad existente por nombre + tipo
  ...
  if (existing.length > 0) {
    // Actualizar CUIT si teníamos null y ahora hay dato
...
async function persistExtraction(
  ...
  await db.transaction(async (tx) => {
    ...
    // 2. INSERT artículos
    if (extraction.articulos.length > 0) {
      await tx.insert(articulos).values(
        extraction.articulos.map((art, idx) => ({
          ordenanzaId,
    ...
    if (extraction.montos_detallados.length > 0) {
      const montosValues = extraction.montos_detallados
        .filter((m) => m.valor !== null && m.concepto)
        .map((m) => ({
          ordenanzaId,
          concepto: m.concepto,
...
async function main() {
  console.log("=== Procesador de Ordenanzas con Gemini API ===\n");
