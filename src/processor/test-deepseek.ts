...
async function main() {
  console.log("=== Probando DeepSeek-V3 (deepseek-chat) ===\n");
  ...
  try {
    // 3. Llamar a la API
    // Usamos 'deepseek-chat' que corresponde a V3 (Non-thinking)
    ...
    // Mostrar raw text si falla el parseo
    if (error instanceof Error && error.message.includes("Validación Zod")) {
       console.log("Validation failed. Check raw output if possible.");
    }
  } finally {
    await pool.end();
  }
